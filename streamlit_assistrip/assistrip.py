...
# # Download 'punkt' to a writable directory
...
# Load data with error andling
try:
    data = pd.read_csv(file_path)
    data = data[['diss_nm', 'diag_nm', 'stty_infcd_tpcd']].dropna()  # Load necessary columns and drop NaN rows
...
# Define noun extraction function using regex
tokenizer = RegexpTokenizer(r'\b\w+\b')

def extract_nouns(text):
    nouns = tokenizer.tokenize(text)  # Extract words only
    return ' '.join(nouns)

# Create 'combined_nouns' column by combining 'diss_nm' and 'diag_nm'
data['combined_text'] = data['diss_nm'] + ' ' + data['diag_nm']
data['combined_nouns'] = data['combined_text'].apply(extract_nouns)
...
if menu == "법정감염병코드검색":
    ...
    if test_sentence:
        # Extract nouns and compute cosine similarity
        test_sentence_nouns = extract_nouns(test_sentence)
        test_sentence_tfidf = tfidf_vectorizer.transform([test_sentence_nouns])
        cosine_similarities = cosine_similarity(test_sentence_tfidf, tfidf_matrix)
        most_similar_index = cosine_similarities.argmax()
        ...
        # Handle cases with zero similarity
        if similarity_score == 0.0:
            most_similar_code = 4
